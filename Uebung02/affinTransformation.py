...
class AffinTransformation:
    ...
    def __init__(self):
        print("init AffinTransformation")

    def transform(self):
        img = self.read_image('schraegbild_tempelhof.jpg', True)
        ...
        img = self.f_affin_transformation(A, 0, img, 'bil', rgb=False)
        self.plot_image(img)



        img = self.read_image('schraegbild_tempelhof.jpg', False)
        ...
        img = self.f_affin_transformation(A, 0, img, 'bil', rgb=True)
        self.plot_image(img)


    def f_affin_transformation(self,A, a0, src, method, rgb):
        ...
        sx, sy = self.rotate_coords(dx + cx.min(), dy + cy.min(), A, ox, oy)
    ...
    def rotate_coords(self,x, y, A, ox, oy):
